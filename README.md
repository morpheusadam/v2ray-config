...
```mermaid
flowchart LR
    A["Search GitHub<br/>12 queries"] --> B["Read every repo<br/>README · file tree"]
    B --> C["Collect candidate<br/>links"]
    C --> D{"Fetch it.<br/>Does it carry<br/>configs?"}
    D -->|no| E["Recorded, not published"]
    D -->|yes| F["Dial a sample of<br/>its servers over TCP"]
    F --> G["Score: reach · freshness ·<br/>uniqueness · speed · volume"]
    G --> H["subs/all.txt<br/>best first"]
    E -.->|"12 days quiet"| I["retired.txt"]
    H -.->|"12 days unchanged"| I
```
